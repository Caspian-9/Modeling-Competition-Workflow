...
- `cases/<case_id>/initialization/`：case 级数据预处理、探索性分析、EDA 图与摘要前正文；默认在 Q1 前完成。
...
11. raw intake 后默认先完成 `initialization/plan.md -> engineer_report.md -> summary.md -> figures/ -> front_matter.md`；除非用户明确跳过，不在初始化未完成时开始 Q1。

## 探索与检验责任

- 建模前先按 `知识库/00_规范与索引/问题驱动入口.md` 读取相关探索或检验卡，再按需查看模型卡；知识库不是方法白名单。
- 主 Agent 在当前 `analysis.md` 记录任务目标、数据生成与信息边界、探索发现及其对模型的影响、关键假设与反例。无数据题以量纲、机制、边界和可解小例代替数据 EDA。
- 在拟合、调参或正式求解前明确基线、验证设计、指标或约束检查以及失败后的处置。预测任务先隔离最终测试信息，不能用测试结果指导探索和选择。
- 采用结论前，将主张与实际检验证据对应，区分“未检查、通过、失败、不适用”；通过须有证据，不适用须解释。核心检验缺失或失败时补实验、修改模型或缩小结论，不写成已经验证。
- 探索与检验按问题风险选取，不要求固定模型数量、统一检验套餐或每步审批。允许库外方法、失败探索和简短分析；复杂性须回应已发现的问题。
- Skill 决定何时行动，知识卡提供条件与方法，Harness 保留复现证据，主 Agent 负责判断。标题齐全或 QC 通过不等于科学有效。
...
初始化由主 Agent 规划、Engineer 实施数据审计/预处理/EDA、主 Agent 总结、Writer Figure Skill 绘制中文 EDA 图、Writer 起草摘要前正文。完成后向用户展示 `initialization/front_matter.md` 和 EDA 图，记录审阅意见后再进入 Q1。
